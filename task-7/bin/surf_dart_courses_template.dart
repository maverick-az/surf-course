...
void main(List<String> arguments) {
  ...
  final List<RawProductItem> filteredRawProducts =
      rawProducts.where((rawProduct) {
    return rawProduct.expirationDate.compareTo(DateTime.now()) > 0 &&
        rawProduct.qty > 0;
  }).toList();

  // Формирование каталога товаров
  final RawProductCatalog rawProductCatalog = {};
  filteredRawProducts.fold(rawProductCatalog, (catalog, rawProduct) {
    catalog
        .putIfAbsent(
            rawProduct.categoryName, () => <String, List<RawProductItem>>{})
        .putIfAbsent(rawProduct.subcategoryName, () => <RawProductItem>[])
        .add(rawProduct);
    return catalog;
  });

  print(rawProductCatalog);
}
